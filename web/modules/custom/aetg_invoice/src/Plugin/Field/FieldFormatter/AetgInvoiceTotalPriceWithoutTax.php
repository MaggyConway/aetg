...
/**
 * Plugin implementation of the 'aetg_commerce_invoice_total_price_without_tax' formatter.
 *
 * @FieldFormatter(
 *   id = "aetg_commerce_invoice_total_price_without_tax",
 *   label = @Translation("Invoice total price without tax"),
 *   field_types = {
 *     "commerce_price",
 *   },
 * )
 */
class AetgInvoiceTotalPriceWithoutTax extends FormatterBase implements ContainerFactoryPluginInterface {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    $elements = [];
    if (!$items->isEmpty()) {
      $total_tax = $this->invoiceService->getTotalTaxAmount($invoice);
      $total_without_tax = $invoice->getTotalPrice()->subtract($total_tax);

      $elements[0] = [
        '#theme' => 'aetg_commerce_invoice_total_price_without_tax',
