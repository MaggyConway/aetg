 ...
 *
 * @MigrateSource(
 *   id = "d7_associate_profile2",
 *   source_module = "profile2"
 * )
 */
class AetgProfile2 extends FieldableEntity {
  ...
  public function prepareRow(Row $row) {
    ...
    foreach (array_keys($this->getFields('user')) as $field) {
      $tid = $row->getSourceProperty('uid');
      $row->setSourceProperty($field, $this->getFieldValues('user', $field, $tid));
    }
