...
/**
 * Sends a confirmation email when an invoice is generated && the order is free.
 *
 * @see \Drupal\commerce_invoice\EventSubscriber\InvoiceConfirmationSubscriber
 */
class InvoicePaidSubscriber implements EventSubscriberInterface, DestructableInterface {
  ...
  /**
   * Sends an invoice confirmation email if the order is free.
   *
  ...
  public function destruct() {
    ...
    foreach ($invoices as $invoice) {
      ...
      if ($invoice->isPaid() && $invoice->getTotalPrice()->isZero()) {
        /** @var \Drupal\commerce_invoice\Entity\InvoiceTypeInterface $invoice_type */
        $invoice_type = $invoice_type_storage->load($invoice->bundle());
